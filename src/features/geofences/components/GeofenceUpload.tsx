...
export function GeofenceUpload({ isOpen, onClose, refetch, geofenceTypes }: GeofenceUploadProps) {
    ...
    const handleUpload = async () => {
        ...
        if (!name.trim()) {
            toast.error('Nombre requerido', {
                description: 'Por favor ingresa un nombre para la geocerca.',
            });
            return;
        }
        ...
        if (!selectedTypeId) {
            toast.error('Tipo requerido', {
                description: 'Por favor selecciona un tipo de geocerca.',
            });
            return;
        }
        ...
        try {
            await geofencesApiService.uploadGeofence(file, name.trim(), selectedTypeId);
            toast.success('Geocerca creada exitosamente', {
                description: `La geocerca "${name.trim()}" ha sido registrada correctamente en el sistema.`,
            });
            setFile(null);
            setName('');
            setSelectedTypeId('');
            ...
            toast.error('Error al subir la geocerca', {
            ...
            <div className="relative bg-white dark:bg-[#0A1628] border border-gray-300 dark:border-[#004599]/30 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col animate-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="flex items-center justify-between p-6 pb-4 border-b border-gray-200 dark:border-gray-700">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-bold text-gray-900 dark:text-white">Subir Geocerca</h2>
                    </div>
                    <button
                        onClick={handleClose}
                        disabled={uploading}
                ...
                <div className="flex-1 overflow-y-auto px-6 py-4">
                    ...
                    <div>
                        <label htmlFor="geofence-name" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    ...
                    <div>
                        <label htmlFor="geofence-type" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Tipo de geocerca *
                        </label>
                        <select
                            id="geofence-type"
                            ...
                            disabled={uploading}
                            className="w-full px-3 py-2 bg-gray-50 dark:bg-black/30 border border-gray-300 dark:border-[#004599]/30 rounded-lg text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent disabled:opacity-50 transition-all cursor-pointer"
                        >
                            <option value="" className="bg-white dark:bg-[#0A1628]">Selecciona un tipo</option>
                            {geofenceTypes.map((type) => (
                                <option key={type.id} value={type.id} className="bg-white dark:bg-[#0A1628]">
                                    {type.name}
                                </option>
                            ))}
                        </select>
                ...
                <div className="p-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <Button
                        ...
                        disabled={!file || !name.trim() || !selectedTypeId || uploading}
                        isLoading={uploading}
